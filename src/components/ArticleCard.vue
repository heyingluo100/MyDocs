...
const isUnlocked = computed(() => isArticleUnlocked(props.article.slug))
...
<template>
  ...
  >
    ...
    <h3 class="text-base font-semibold text-linear-text group-hover:text-linear-accent transition-colors mb-2 flex items-center gap-1.5">
      ...
      <svg v-if="article.locked && !isUnlocked" class="w-3.5 h-3.5 text-red-800/60 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
